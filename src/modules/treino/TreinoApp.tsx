...
import { getSupabase, isSupabaseConfigured, getSupabaseConfigProblem } from "../../lib/supabase";
...
const ACHIEV=[
  {id:"first",icon:"⭐",title:"Primeiro treino!",desc:"Completou o primeiro treino",check:l=>gymSessions(l)>=1},
  ...
  {id:"c1",icon:"🔄",title:"Ciclo completo",desc:"5 treinos do ciclo",check:l=>gymSessions(l)>=5},
  {id:"w10",icon:"💪",title:"10 treinos",desc:"Dedicação!",check:l=>gymSessions(l)>=10},
  {id:"w25",icon:"👑",title:"25 treinos",desc:"Rainha do ferro",check:l=>gymSessions(l)>=25},
  {id:"w50",icon:"🏋️",title:"50 treinos",desc:"Volume impressionante",check:l=>gymSessions(l)>=50},
  {id:"w75",icon:"⚔️",title:"75 treinos",desc:"Quase lá dos 100",check:l=>gymSessions(l)>=75},
  {id:"w100",icon:"💯",title:"100 treinos",desc:"Centenário de treinos",check:l=>gymSessions(l)>=100},
  ...
  {id:"mix3",icon:"🔀",title:"Três pilares",desc:"Pelo menos um treino, uma corrida e uma atividade livre (no total)",check:l=>l.some(e=>e.type==="workout"||e.type==="quick")&&l.some(e=>e.type==="run")&&l.some(e=>e.type==="free")},
  {id:"prog",icon:"📈",title:"1ª progressão",desc:"Aumentou carga num exercício",check:l=>{const w={};for(const e of l){if((e.type==="workout"||e.type==="quick"||e.type==="home")&&e.w)for(const[k,v]of Object.entries(e.w)){const x=weightFromLogVal(v);if(x===undefined)continue;if(!w[k])w[k]=[];w[k].push(x);}}return Object.values(w).some(a=>a.length>1&&a[a.length-1]>a[0]);}},
];
...
"q-hip-thrust-smith":{img:`${GFP}/images/exercises/mixed/hip-thrust.jpg`,page:`${GFP}/exercises/barbell-hip-thrust`,steps:["Na Smith, barra sobre o quadril; escápulas apoiadas no banco atrás.","Pés firmes no chão na largura do quadril.","Eleve o quadril até alinhar tronco e coxas; pausa 1–2 s no topo.","Desça com controle sem perder a lombar neutra."]},
"q-agach-smith":{img:`${GFP}/images/exercises/mixed/squat-smith-machine.jpg`,page:`${GFP}/exercises/smith-machine-squats`,steps:["Pés um pouco à frente da barra para joelhos confortáveis.","Desça no trilho mantendo costas apoiadas no movimento.","Quadril e joelhos flexionam juntos; amplitude conforme mobilidade.","Suba sem travar agressivamente os joelhos."]},
"q-remada-smith":{img:`${GFP}/images/exercises/mixed/cable-seated-row.jpg`,page:`${GFP}/exercises/seated-cable-back-rows`,steps:["Barra da Smith na altura do baixo do peito; incline o tronco com costas neutras.","Puxe a barra em direção ao abdômen, cotovelos para trás.","Contraia as escápulas no final.","Estenda os braços com controle."]},
"q-supino-smith":{img:`${GFP}/images/exercises/mixed/bench-press-d.jpg`,page:`${GFP}/exercises/dumbbell-bench-press`,steps:["Banco plano sob a barra da Smith; pegada um pouco mais larga que os ombros.","Escápulas fixas; desça a barra ao peito com controle.","Empurre para cima em linha estável; cotovelos ~45°.","Evite arquear demais a lombar."]},
"q-cross-fly":{img:`${GFP}/images/exercises/mixed/cable-fly-cross-over.jpg`,page:`${GFP}/exercises/standing-cable-crossover-flyes`,steps:["Cabos em altura média; passo leve à frente.","Abra os braços com cotovelos levemente flexionados.","Feche em arco até a linha média do peitoral.","Volte com tensão constante."]},
"q-cross-tri":{img:`${GFP}/images/exercises/mixed/triceps-pushdown.jpg`,page:`${GFP}/exercises/triceps-pushdown`,steps:["Cabo alto; cotovelos fixos junto ao tronco.","Estenda os antebraços para baixo até o tríceps contrair.","Não abra os cotovelos para os lados.","Volte até ~90° com tronco firme."]},
"h-glute-bridge":{img:`${GFP}/images/exercises/mixed/hip-thrust.jpg`,page:`${GFP}/exercises/barbell-hip-thrust`,steps:["Deitada, joelhos flexionados, pés no chão.","Elástico acima dos joelhos (opcional).","Eleve o quadril até alinhar ombros e joelhos; segure 1–2 s.","Desça sem perder a contração dos glúteos."]},
"h-clam":{img:`${GFP}/images/exercises/mixed/seated-abductor.jpg`,page:`${GFP}/exercises/seated-abductor-machine`,steps:["De lado no chão, joelhos flexionados e empilhados.","Abra o joelho de cima como uma concha, mantendo os pés juntos.","Não role o quadril para trás.","Volte com controle; troque de lado."]},
"h-squat-bw":{img:`${GFP}/images/exercises/mixed/squat-bar.jpg`,page:`${GFP}/exercises/barbell-squats`,steps:["Pés na largura do quadril; elástico acima dos joelhos se usar.","Agache empurrando joelhos para fora sobre os pés.","Excêntrico controlado (~3 s).","Suba estendendo quadril e joelhos juntos."]},
"h-lunge-bw":{img:`${GFP}/images/exercises/mixed/bulgarian-split-squat-dumbbell.jpg`,page:`${GFP}/exercises/bulgarian-dumbbell-split-squat`,steps:["Em pé, dê um passo à frente longo o suficiente.","Desça até joelhos ~90°; joelho de trás quase toca o chão.","Empurre o chão com o pé da frente para voltar.","Alterne as pernas."]},
"h-pushup":{img:`${GFP}/images/exercises/mixed/bench-press-d.jpg`,page:`${GFP}/exercises/push-up`,steps:["Mãos sob os ombros; corpo em prancha (ou joelhos no chão).","Desça o peito em direção ao chão; cotovelos ~45°.","Mantenha abdômen firme e quadril alinhado.","Empurre até estender sem travar."]},
"h-band-row":{img:`${GFP}/images/exercises/mixed/cable-seated-row.jpg`,page:`${GFP}/exercises/seated-cable-back-rows`,steps:["Prenda o elástico à frente; segure as pontas.","Puxe em direção ao tronco com cotovelos para trás.","Aperte as escápulas no final.","Estenda os braços mantendo postura."]},
"h-band-pull":{img:`${GFP}/images/exercises/mixed/facepull-face-pull.jpg`,page:`${GFP}/exercises/face-pull`,steps:["Segure o elástico à frente do peito, braços estendidos.","Abra os braços até formar uma linha com o peitoral (pull apart).","Controle o retorno sem elevar os ombros.","Ótimo para postura."]},
"h-deadbug":{img:`${GFP}/images/exercises/mixed/dead-bug.jpg`,page:`${GFP}/exercises/dead-bug`,steps:["Deitada, braços ao teto, joelhos a 90°.","Estenda braço e perna opostos mantendo a lombar colada.","Alterne os lados com ritmo controlado.","Respire com o abdômen estável."]},
"h-bird-dog":{img:`${GFP}/images/exercises/mixed/dead-bug.jpg`,page:`${GFP}/exercises/dead-bug`,steps:["Quatro apoios; mãos sob ombros, joelhos sob quadris.","Estenda braço e perna opostos até o tronco ficar longo.","Evite rodar o quadril; segure 1–2 s.","Troque de lado com controle."]},
"h-band-squat":{img:`${GFP}/images/exercises/mixed/seated-dumbbell-shoulders-press.jpg`,page:`${GFP}/exercises/seated-dumbbell-overhead-shoulder-press`,steps:["Pise no meio do elástico; segure as pontas à altura dos ombros.","Empurre para cima até quase estender os braços (desenvolvimento com resistência).","Abdômen firme; não hiperextenda a lombar.","Desça devagar até os ombros."]},
};
...
/** Treinos rápidos (~30 min) — mesmo foco muscular do dia na fila, menos exercícios/séries; avançam qi. */
const QUICK=[
  {id:"q-glut",label:"⚡ Glúteos + posterior (rápido)",focus:"Versão reduzida · ~30 min",emoji:"⚡",warmup:["Ponte isométrica 30s","Abdução com banda 15x"],exercises:[{id:"hip-thrust",name:"Hip thrust (barra)",sets:3,reps:10,rest:75,dw:40,tech:"Pausar 2s no topo",cues:"Escápulas no banco / contração firme"},{id:"rdl",name:"RDL (barra)",sets:3,reps:10,rest:60,dw:30,tech:"Excêntrico 3s",cues:"Barra na canela / quadril para trás"},{id:"flexora-abd",name:"Flexora + abdução (SS)",sets:2,reps:12,rest:45,dw:25,tech:"Supersérie",cues:"Flexora → abdução sem descanso entre movimentos"}],finisher:null,posture:["Gato-vaca 8 ciclos","Alongamento do flexor 30s","Pombo 30s"]},
  {id:"q-costas",label:"⚡ Costas + bíceps (rápido)",focus:"Versão reduzida · ~30 min",emoji:"⚡",warmup:["Rotação externa 15x","Gato-vaca 6 ciclos"],exercises:[{id:"lat-pull",name:"Puxada na polia alta",sets:3,reps:10,rest:60,dw:35,tech:"Excêntrico 3s",cues:"Escápulas estáveis"},{id:"remada-baixa",name:"Remada baixa",sets:3,reps:12,rest:60,dw:30,cues:"Puxar até o abdômen"},{id:"face-pull-t",name:"Face pull",sets:3,reps:15,rest:45,dw:15,tech:"Rotação externa",cues:"Cotovelos altos"},{id:"rosca-mm",name:"Rosca martelo",sets:2,reps:12,rest:45,dw:8,cues:"Cotovelos fixos"}],finisher:null,posture:["Postura da criança 45s","Suspensão passiva na barra 2x20s"]},
  {id:"q-peito",label:"⚡ Peito + tríceps (rápido)",focus:"Versão reduzida · ~30 min",emoji:"⚡",warmup:["Alongamento peitoral 30s","Rotação de ombros"],exercises:[{id:"supino-reto",name:"Supino com halteres",sets:3,reps:10,rest:60,dw:12,tech:"Excêntrico 3s",cues:"Escápulas retraídas"},{id:"crossover",name:"Crossover (cabo)",sets:3,reps:12,rest:45,dw:10,cues:"Contração forte no peitoral"},{id:"triceps-polia",name:"Tríceps na polia",sets:3,reps:12,rest:45,dw:15,cues:"⚠️ Cotovelo direito"},{id:"deadbug-ext",name:"Dead bug + elevação de pernas",sets:2,reps:10,rest:30,dw:0,cues:"Lombar colada"}],finisher:null,posture:["Alongamento peitoral na porta 30s","Cobra 3x10s"]},
  {id:"q-quad",label:"⚡ Quadríceps + adutores (rápido)",focus:"Versão reduzida · ~30 min",emoji:"⚡",warmup:["Bicicleta 3 min","Adução leve 15x"],exercises:[{id:"agach-livre",name:"Agachamento livre",sets:3,reps:8,rest:75,dw:40,tech:"Pausar 1s no fundo",cues:"Abdômen firme"},{id:"extensora",name:"Extensora",sets:3,reps:12,rest:45,dw:25,tech:"Rest-pause na última"},{id:"aducao",name:"Adução na máquina",sets:2,reps:15,rest:45,dw:20,tech:"Amplitude parcial"}],finisher:null,posture:["Alongamento do flexor 30s","Pombo 30s","Borboleta 30s"]},
  {id:"q-ombro",label:"⚡ Ombros + glúteo médio (rápido)",focus:"Versão reduzida · ~30 min",emoji:"⚡",warmup:["Rotação externa 15x","Abdução com banda 12x"],exercises:[{id:"desenv",name:"Desenvolvimento com halteres",sets:3,reps:10,rest:60,dw:10,cues:"Cotovelos ~90°"},{id:"lat-raise",name:"Elevação lateral",sets:3,reps:15,rest:45,dw:5,tech:"Excêntrico 3s",cues:"Polegar leve para cima"},{id:"abd-maq-sex",name:"Abdução máq. + caneleira",sets:2,reps:15,rest:45,dw:20,tech:"Supersérie"},{id:"face-pull-sex",name:"Face pull",sets:2,reps:15,rest:45,dw:12,cues:"Postural"}],finisher:null,posture:["Postura da criança 45s","Alongamento do flexor 30s"]},
];
/** Treinos em casa — extra; não avançam a fila (qi). */
const HOME=[
  {id:"h-glute",label:"🏠 Casa: glúteos + core",focus:"Elástico + peso corporal · ~25 min",emoji:"🏠",warmup:["Gato-vaca 8 ciclos","Ponte isométrica 30s"],exercises:[{id:"h-glute-bridge",name:"Ponte de glúteo (elástico)",sets:4,reps:15,rest:45,dw:0,tech:"Pausar 2s no topo; elástico acima dos joelhos",cues:"Pés na largura do quadril / máxima contração"},{id:"h-clam",name:"Clamshell (elástico)",sets:3,reps:15,rest:30,dw:0,tech:"Cada lado",cues:"Quadril estável / sem rodar"},{id:"h-squat-bw",name:"Agachamento (elástico)",sets:3,reps:15,rest:45,dw:0,tech:"Elástico acima dos joelhos; excêntrico 3s",cues:"Joelhos para fora"},{id:"h-lunge-bw",name:"Avanço alternado",sets:3,reps:12,rest:45,dw:0,cues:"Passo longo / joelho ~90°"},{id:"h-deadbug",name:"Dead bug",sets:3,reps:10,rest:30,dw:0,cues:"Lombar colada / lento"},{id:"h-bird-dog",name:"Bird dog",sets:3,reps:10,rest:30,dw:0,cues:"Core ativo / braço e perna opostos"}],finisher:null,posture:["Postura da criança 45s","Pombo 30s","Borboleta 30s","Cobra 3x10s"]},
  {id:"h-upper",label:"🏠 Casa: superior + postura",focus:"Elástico + peso corporal · ~25 min",emoji:"🏠",warmup:["Rotação de ombro 15x","Livro aberto 8x"],exercises:[{id:"h-pushup",name:"Flexão (joelhos ou completa)",sets:3,reps:12,rest:45,dw:0,tech:"Excêntrico 3s",cues:"Corpo alinhado / cotovelos 45°"},{id:"h-band-row",name:"Remada com elástico",sets:3,reps:15,rest:45,dw:0,tech:"Contração 2s",cues:"Escápulas juntas / cotovelos para trás"},{id:"h-band-pull",name:"Pull apart (elástico)",sets:3,reps:15,rest:30,dw:0,cues:"Braços estendidos / abrir até a linha do peito"},{id:"h-band-squat",name:"Desenvolvimento com elástico",sets:3,reps:12,rest:45,dw:0,cues:"Pisar no elástico / empurrar para cima"},{id:"h-deadbug",name:"Dead bug",sets:3,reps:10,rest:30,dw:0,cues:"Lombar colada"},{id:"h-bird-dog",name:"Bird dog",sets:3,reps:10,rest:30,dw:0,cues:"Estabilidade / lento"}],finisher:null,posture:["Alongamento peitoral na porta 30s","Fio da agulha 8x","Cobra 3x10s","Postura da criança 45s"]},
];
const ALL_ROUTINES=[...WP,...QUICK,...HOME];
...
function exNameById(id){const x=ALL_ROUTINES.flatMap(w=>w.exercises).find(e=>e.id===id);return x?.name||id;}
...
function getXP(l){return l.filter(e=>e.type==="workout"||e.type==="quick").length*100+l.filter(e=>e.type==="home").length*50+l.filter(e=>e.type==="free").length*40+l.filter(e=>e.type==="rest").length*10+l.filter(e=>e.type==="run").length*50;}
...
function activeDaysUnion(log,bodyW){const s=activeDaysSet(log);(bodyW||[]).forEach(b=>{if(b?.date&&/^\d{4}-\d{2}-\d{2}$/.test(String(b.date)))s.add(String(b.date).slice(0,10));});return s;}
...
export default function TreinoApp(){
  ...
  const[woAlt,setWoAlt]=useState<{k:"quick"|"home";i:number}|null>(null);
  const[doneXp,setDoneXp]=useState(100);
  ...
  const load=useCallback(async()=>{try{const{data,session,syncNote:note}=await loadTreinoState();setData(data);setSyncNote(note??null);setAuthUser(session?.user?{id:session.user.id,email:session.user.email??undefined}:null);prevAch.current=new Set(data.ach||[]);if(session?.user){try{await persistTreinoData(data);}catch(e){console.warn("[treino] enviar para nuvem ao abrir:",e);}}}catch(e){const i=initTreinoData();try{await persistTreinoData(i);}catch(e2){}setData(i);setAuthUser(null);setSyncNote(null);prevAch.current=new Set([]);}setLoading(false);},[]);
  useEffect(()=>{void load();},[load]);
  useEffect(()=>{const sb=getSupabase();if(!sb)return;const{data:{subscription}}=sb.auth.onAuthStateChange(async(event,session)=>{setAuthUser(session?.user?{id:session.user.id,email:session.user.email??undefined}:null);if(event==="TOKEN_REFRESHED"||event==="USER_UPDATED")return;if(!session?.user)return;try{const{data,syncNote:note}=await loadTreinoState();setData(data);setSyncNote(note??null);prevAch.current=new Set(data.ach||[]);if(event==="SIGNED_IN"||event==="INITIAL_SESSION"){try{await persistTreinoData(data);}catch(e){console.warn("[treino] sync após auth:",e);}}}catch(e){console.warn(e);}});return()=>subscription.unsubscribe();},[]);
  ...
  function gw(id){
    ...
    const ex=ALL_ROUTINES.flatMap(w=>w.exercises).find(e=>e.id===id);
    return ex?.dw??0;
  }
  ...
  function wH(id){return(data?.log||[]).filter(e=>(e.type==="workout"||e.type==="quick")&&e.w?.[id]!==undefined).map(e=>({date:e.date,weight:weightFromLogVal(e.w[id])})).filter(e=>e.weight!==undefined);}
  function wkCt(){const s=mondayThisWeekISO();return(data?.log||[]).filter(e=>(e.type==="workout"||e.type==="quick")&&e.date>=s).length;}
  function totW(){return(data?.log||[]).filter(e=>e.type==="workout"||e.type==="quick").length;}
  function todayHasW(){return(data?.log||[]).some(e=>e.date===td()&&(e.type==="workout"||e.type==="quick"||e.type==="home"));}
  ...
  async function complete(wo,weights,fb,logType="workout"){
    ...
    const advanceQi=logType==="workout"||logType==="quick";
    const nd={...data,log:[...(data.log||[]),{type:logType,date:td(),label:wo.label,wid:wo.id,w:wLog,fb:fb?.tags?.length||fb?.note?{tags:fb.tags||[],note:fb.note||""}:null,skipped:skipped.length?skipped:undefined}],ew:ewNext,qi:advanceQi?(data.qi||0)+1:(data.qi||0)};
    setDoneXp(logType==="home"?50:100);
  ...
  async function saveBodyProfile(p:{age:number;heightCm:number}){await save({...data,profile:p});}
  async function saveBodyEntry(e:BodyWeeklyEntry){const list=[...(data.bodyWeekly||[])].filter(x=>x.date!==e.date);list.push(e);list.sort((a,b)=>a.date.localeCompare(b.date));await save({...data,bodyWeekly:list});}
  ...
  if(view==="body")return(<div style={S.root}>{ap}<BodyCompositionView profile={data.profile??{age:30,heightCm:165}} entries={data.bodyWeekly??[]} onSaveProfile={saveBodyProfile} onSaveEntry={saveBodyEntry} onDeleteEntry={delBodyEntry} onBack={()=>setView("home")}/></div>);
  ...
  const estAltQuick=QUICK.map(q=>estimateWorkoutMinutes(q));
  ...
  return(<div style={S.root}>{ap}
    ...
    {isSupabaseConfigured()&&(syncNote||authUser)&&<div style={S.syncBox}>{syncNote?(<><div style={S.syncRow}><span style={{fontSize:18}}>⚠️</span><p style={S.syncWarnT}>{syncNote}</p></div><p style={S.syncSub}>No Supabase: Authentication → Sign In / Providers → ative &quot;Anonymous sign-ins&quot;. Até lá os dados ficam só neste aparelho.</p></>):(<><p style={{...S.syncOk,margin:0,lineHeight:1.45}}>☁️ Nuvem ligada — sincronização automática</p><p style={S.syncSub}>Ao abrir e ao gravar, os dados sincronizam com o projeto.{authUser?.email?` Conta: ${authUser.email}.`:""}</p></>)}</div>}
    ...
    <div style={S.nav}><button style={S.nB} onClick={()=>{setWoAlt(null);setView("queue");}}><span style={S.nI}>📋</span>Fila</button><button style={S.nB} onClick={()=>setView("progress")}><span style={S.nI}>📈</span>Progressão</button><button style={S.nB} onClick={()=>setView("body")}><span style={S.nI}>📷</span>Corpo</button><button style={S.nB} onClick={()=>setView("runProg")}><span style={S.nI}>🏃</span>Corridas</button><button style={S.nB} onClick={()=>setView("history")}><span style={S.nI}>📅</span>Histórico</button><button style={S.nB} onClick={()=>setView("achiev")}><span style={S.nI}>🏆</span>Conquistas</button></div>
  </div>);
}
...
const S={
  ...
  syncRow:{display:"flex",alignItems:"flex-start",gap:10,flexWrap:"wrap"},
