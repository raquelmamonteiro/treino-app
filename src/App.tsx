...
/** Treinos rápidos (Smith + crossover) — ao concluir, avançam a fila como o treino principal. */
const QUICK=[
  {id:"q-lower",label:"⚡ Rápido: parte inferior",focus:"Smith + crossover · ~35 min",emoji:"⚡",warmup:["Ponte isométrica 30s","Agachamento sem peso 10x"],exercises:[{id:"q-hip-thrust-smith",name:"Hip thrust na Smith",sets:4,reps:10,rest:75,dw:40,tech:"Pausar 2s no topo",cues:"Pés firmes / contração de glúteo"},{id:"q-agach-smith",name:"Agachamento na Smith",sets:4,reps:10,rest:75,dw:30,tech:"Excêntrico 3s",cues:"Pés à frente / amplitude segura"},{id:"rdl",name:"RDL (barra)",sets:3,reps:10,rest:60,dw:30,tech:"3s descendo",cues:"Quadril para trás / costas neutras"},{id:"bulgaro",name:"Búlgaro (peso corporal ou barra)",sets:3,reps:12,rest:60,dw:0,cues:"Banco atrás / tronco inclinado"}],finisher:null,posture:["Gato-vaca 8 ciclos","Alongamento do flexor 30s","Pombo 30s cada lado"]},
  {id:"q-upper",label:"⚡ Rápido: parte superior",focus:"Smith + crossover · ~35 min",emoji:"⚡",warmup:["Rotação externa 15x","Alongamento peitoral 30s"],exercises:[{id:"q-supino-smith",name:"Supino na Smith",sets:4,reps:10,rest:75,dw:20,tech:"Excêntrico 3s",cues:"Escápulas retraídas / cotovelos 45°"},{id:"q-remada-smith",name:"Remada curvada na Smith",sets:4,reps:10,rest:75,dw:25,tech:"Contração 1s",cues:"Cotovelos para trás / peito aberto"},{id:"q-cross-fly",name:"Crucifixo no crossover",sets:3,reps:12,rest:60,dw:8,cues:"Cotovelos levemente flexionados"},{id:"q-cross-tri",name:"Tríceps no crossover",sets:3,reps:12,rest:45,dw:12,tech:"Drop na última série",cues:"⚠️ Cotovelo direito"},{id:"rosca-mm",name:"Rosca com barra",sets:3,reps:12,rest:45,dw:15,cues:"Cotovelos fixos"}],finisher:null,posture:["Alongamento peitoral na porta 30s","Livro aberto 8x","Cobra 3x10s"]},
];
  ...
  return(<div style={S.root}>{ap}
    ...
    <div style={S.sec}><h2 style={S.sT}>Próximo treino</h2>
      <div style={S.nC}>
        ...
        <p style={{...S.quickHint,marginTop:0}}>Rápidos (Smith + crossover) substituem o treino da fila. Em casa são extra e não avançam a fila.</p>
        <div style={{display:"flex",gap:8,marginBottom:8}}>{QUICK.map((q,i)=>(<div key={q.id} style={{flex:1,background:"#161321",borderRadius:16,padding:12,border:"1px solid #f59e0b33",cursor:"pointer",minWidth:0}} onClick={()=>{setFbNote("");setFbTags([]);setWoAlt({k:"quick",i});setQuickRun(false);setView("workout");}}><div style={{fontSize:20,marginBottom:4}}>{q.emoji}</div><div style={{fontWeight:800,fontSize:11,lineHeight:1.25}}>{q.label.replace(/^⚡ Rápido:\s*/,"")}</div><div style={{fontSize:9,color:"#f59e0b",marginTop:4,fontWeight:700}}>Substitui o próximo</div><div style={{fontSize:9,color:"#6b6280",marginTop:2}}>~{estAltQuick[i]} min</div></div>))}</div>
